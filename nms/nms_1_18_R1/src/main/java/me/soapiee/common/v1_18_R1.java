...
class v1_18_R1 implements NMSProvider {
    ...
    public boolean setSpectator(Player player) {
        ...
        p.connection.send(new ClientboundGameEventPacket(ClientboundGameEventPacket.CHANGE_GAME_MODE, 3)); //1 = creative 2 = survival 3 = spectator
    ...
    public void updateTab(Player player, HashSet<UUID> spectators) {
        ...
        for (UUID uuid : spectators) {
            ...
            ArrayList<ClientboundPlayerInfoPacket.PlayerUpdate> list = new ArrayList<>();
            list.add(new ClientboundPlayerInfoPacket.PlayerUpdate(spec.getBukkitEntity().getProfile(), 0, GameType.SURVIVAL, null));
